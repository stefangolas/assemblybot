"""Scoped raw-CDP page helper bound to ONE target id, so this scout never drives
another scout's tab. Mirrors data/rawcdp.Page but selects by exact target id."""
...
def ws_url_for_id(target_id: str, port: int = 9222) -> str:
    d = json.load(urllib.request.urlopen(f"http://127.0.0.1:{port}/json/list", timeout=8))
    for t in d:
        if t.get("id") == target_id:
            return t["webSocketDebuggerUrl"]
    raise RuntimeError(f"no target with id {target_id}")


class TabPage:
    def __init__(self, target_id: str, port: int = 9222, timeout: int = 120):
        self.target_id = target_id
        self.ws = websocket.create_connection(ws_url_for_id(target_id, port),
